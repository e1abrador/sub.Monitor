...
def insert_subdomains(subdomains, out_of_scope_domains=[], manually=False):
    ...
    def is_out_of_scope(subdomain):
        return any(
            fnmatch.fnmatch(subdomain, pattern) for pattern in out_of_scope_domains
        )
...
def list_domains():
    ...
    for domain, (manually, discovered) in domains.items():
        print(
            f"{domain} [{manually + discovered} total unique in database] [{manually} subdomains added manually] [{discovered} subdomains discovered]"
        )
...
def notify(subdomain, domain):
    ...
    command = f'echo "Subdomain https://{subdomain} has been discovered and is live!" | {notify_binary} -silent -config {notify_api}'
    subprocess.run(
...
def is_live(subdomain):
    ...
    command = f"{httpx_binary} -silent -sc -title -u {subdomain}"
...
def dump_subdomains(domain, show_info=False, inscope=False):
    with sqlite3.connect(DATABASE) as conn:
        ...
        for result in results:
            ...
            if (
                inscope and out_of_scope_db
            ):  # if the --inscope flag is provided, and the subdomain is out of scope, then skip it
                continue
            ...
            if show_info:
                print(
                    f"{subdomain} [discovered on {discovered_on}] {out_of_scope_string}"
                )
            else:
...
def main():
    ...
    parser.add_argument(
        "--dump", action="store_true", help="Dump all subdomains for a specific domain"
    )
    parser.add_argument(
        "--list", action="store_true", help="List all root domains in the database"
    )
    parser.add_argument("-df", help="File with domains to scan")
    parser.add_argument(
        "--inscope", action="store_true", help="Dump only in-scope domains"
    )
    parser.add_argument(
        "--notinscope", action="store_true", help="Dump only out-of-scope domains"
    )
    parser.add_argument(
        "--dumpall", action="store_true", help="Dump all subdomains with their details"
    )
    parser.add_argument(
        "--info", action="store_true", help="Show discovery date for subdomains"
    )
    parser.add_argument(
        "-help",
        "-?",
    ...
    if args.add and args.file:
        ...
        with open(args.file) as f:
            num_inserted = insert_subdomains(
                f.read().splitlines(),
                out_of_scope_domains=out_scope_rules,
                manually=True,
            )
            print(
                f"[{datetime.datetime.now()}] - {num_inserted} subdomains were added to the local database."
            )
                ...
                with open(output_file) as f:
                    for line in f:
                        ...
                        if new_subdomain not in known:
                            # subdomain is live
                            if is_live(new_subdomain):
                                notify(new_subdomain, domain)
                                print(
                                    f"[{datetime.datetime.now()}] - New live subdomain {new_subdomain} discovered"
                                )
        ...
        dump_subdomains(
            args.d, show_info=args.info, inscope=args.inscope
        )  # Pass the --inscope flag value to dump_subdomains
